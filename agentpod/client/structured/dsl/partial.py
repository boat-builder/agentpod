...
from typing import Any, Generic, NoReturn, Optional, TypeVar, get_args, get_origin
...
def _make_field_optional(
    ...
    if get_origin(annotation) is not None:
        ...
        modified_args = tuple((Partial[arg, MakeFieldsOptional] if isinstance(arg, type) and issubclass(arg, BaseModel) else arg) for arg in generic_args)  # type: ignore[valid-type]
...
class PartialBase(Generic[T_Model]):
    ...
    def get_partial_model(cls) -> type[T_Model]:
        ...
        assert issubclass(cls, BaseModel), f"{cls.__name__} must be a subclass of BaseModel"

        return create_model(
            __model_name=(cls.__name__ if cls.__name__.startswith("Partial") else f"Partial{cls.__name__}"),
            __base__=cls,
            __module__=cls.__module__,
            **{field_name: _make_field_optional(field_info) for field_name, field_info in cls.model_fields.items()},
        )  # type: ignore[all]
    ...
    @classmethod
    def model_from_chunks(cls, json_chunks: Iterable[Any], **kwargs: Any) -> Generator[T_Model, None, None]:
        potential_object = ""
