...
class ParallelBase:
    def __init__(self, *models: type[OpenAISchema]):
        ...
        self.models = models
        self.registry = {model.__name__ if hasattr(model, "__name__") else str(model): model for model in models}
    ...
    def from_response(
        ...
        for tool_call in response.choices[0].message.tool_calls:
            ...
            yield self.registry[name].model_validate_json(arguments, context=validation_context, strict=strict)
...
def handle_parallel_model(typehint: type[Iterable[T]]) -> list[dict[str, Any]]:
    ...
    return [{"type": "function", "function": openai_schema(model).openai_schema} for model in the_types]
